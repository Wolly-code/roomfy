...
def login(request):
    if request.method == 'POST':
        ...
        if user is None:
            return JsonResponse({'error': {'code':'USER_NOT_FOUND','message':'Authentication failed'}}, status=400)
        else:
            try:
...
def signup(request):
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({'error': {'code':'USER_EXISTS','message':'That username has already been taken. Please choose a new username'}}, status=400)
